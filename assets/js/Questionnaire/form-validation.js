...
// Put this helper near the other helpers in form-validation.js
function estimateCameraPlan() {
  // read what the user picked
  const areas = Array.from(document.querySelectorAll('[name="priority_areas[]"]:checked')).map(el => el.value);
  const floors = (document.querySelector('[name="floors"]')?.value || "").trim();
  const size   = (document.querySelector('[name="home_size"]')?.value || "").trim();
  ...
  if (/3\+/.test(floors)) cameraCount += 2;
  if (/Large|Extra Large/i.test(size)) cameraCount += 1;

  // clamp and map to NVR size
  cameraCount = Math.min(Math.max(cameraCount, 2), 16);
  const nvrChannel = cameraCount <= 4 ? 4 : cameraCount <= 8 ? 8 : 16;

  return { cameraCount, nvrChannel, locations: areas };
}

// REPLACE your captureSubmissionValues() with this:
function captureSubmissionValues() {
  ...
  let f = first, l = last;
  if (!first && !last) {
    const full  = getVal('[name="full_name"]') || getVal('[name*="Full Name"]') || "";
    const parts = full.trim().split(/\s+/);
    f = parts.shift() || "";
    l = parts.join(" ") || "";
  }
  ...
  // NEW: compute a plan safely (no undefined vars)
  const plan = estimateCameraPlan();
  ...
  sessionStorage.setItem(
    ...
    JSON.stringify({
      ...
      lead_tier,
      auto_notes: notes,
      // these are now always defined numbers/arrays
      cameraCount: plan.cameraCount,
      cameraRecommendedLocations: plan.locations,
      nvrChannel: plan.nvrChannel,
    })
  );

  // keep Smart Forms hidden fields in sync
  const scoreEl = document.getElementById("lead_score");
  const tierEl  = document.getElementById("lead_tier");
  const noteEl  = document.getElementById("auto_notes");
  if (scoreEl) scoreEl.value = lead_score;
  if (tierEl)  tierEl.value  = lead_tier;
  if (noteEl)  noteEl.value  = notes;

  // also persist a light fallback for the thank-you copy logic
